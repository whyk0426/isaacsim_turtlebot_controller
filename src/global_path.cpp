...
GlobalPath::GlobalPath() : cmp([](const std::shared_ptr<Node> &a, const std::shared_ptr<Node> &b)
...
void GlobalPath::wayPoint()
{
    ...
    // dwa로만 계속 가면 중간에 재 탐색 과정 없어서 새로운 장애물에 대처X
    if ((path_size > 65) && (row[0].empty()))
    {
        for (size_t i = 1; i * 40 < path_size; i++)
        {
            row[0].push_back(path_result[0][40 * i] * map.info.resolution + map.info.origin.position.x);
            row[1].push_back(path_result[1][40 * i] * map.info.resolution + map.info.origin.position.y);
        }
    }
...
std::array<std::vector<int>, 2> GlobalPath::path_()
{
    ...
    while (!open_list.empty())
    {
        ...
        {
            while (current)
            { // current != nullptr 이라는 뜻. 즉 시작 점까지 올라갈때 까지
        ...
        for (int dx = -1; dx <= 1; dx++)
        {
            for (int dy = -1; dy <= 1; dy++)
            {
                ...
                if (nx < 0 || ny < 0 || nx >= (int)grid_map[0].size() || ny >= (int)grid_map.size())
                    continue;
                if (grid_map[ny][nx] > 50)
                    continue;
                if (closed_list.find({nx, ny}) != closed_list.end())
                    continue; // 결과가 .end()가 아니면 이미 방문했단 뜻
    ...
    // if (path[0].back() != goal_xc || path[1].back() != goal_yc){
    //     double min_dist = 1e9;
    //     int best_idx = 0;
    //     for (size_t i = 0; i < path[0].size(); i++){
    //         double wx = path[0][i] * map.info.resolution + map.info.origin.position.x;
    //         double wy = path[1][i] * map.info.resolution + map.info.origin.position.y;
    //         double dist = sqrt((goal_x - wx) * (goal_x - wx) + (goal_y - wy) * (goal_y - wy));
    //         if (dist < min_dist){
    //             min_dist = dist;
    //             best_idx = i;
    //         }
    //     }
    //     goal_xc = path[0][best_idx];
    //     goal_yc = path[1][best_idx];
    // }
...
void GlobalPath::RealToGrid()
{
    ...
    if (goal_xc < 0 || goal_yc < 0 || goal_xc >= width || goal_yc >= height || grid_map[goal_yc][goal_xc] > 50)
    {
        double min_dist = 1e9;
        int best_x = robot_xc;
        int best_y = robot_yc;
